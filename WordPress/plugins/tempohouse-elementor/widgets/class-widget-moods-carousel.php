...
class THR_Widget_Moods_Carousel extends \Elementor\Widget_Base {

    public function get_name()       { return 'tempohouse-moods-carousel'; }
    public function get_title()      { return 'Spaces Carousel'; }
    public function get_icon()       { return 'eicon-slides'; }
    public function get_categories() { return ['tempohouse']; }
    public function get_keywords()   { return ['moods', 'spaces', 'carousel', 'cafe', 'bar', 'gallery', 'tempo']; }

    public function get_style_depends()  { return ['tempohouse-moods']; }
    public function get_script_depends() { return ['tempohouse-drag', 'tempohouse-moods-js', 'tempohouse-spotlight']; }
    ...
    protected function register_controls() {
        ...
        $this->add_control( 'bleed_line_1', [
            'label'   => 'Bleed Text Line 1',
            'type'    => \Elementor\Controls_Manager::TEXT,
            'default' => 'Creating experiences',
        ] );

        $this->add_control( 'bleed_line_2', [
            'label'   => 'Bleed Text Line 2',
            'type'    => \Elementor\Controls_Manager::TEXT,
        ...
        $this->add_control( 'info_notice', [
            ...
            'raw'             => '<p style="font-size:11px;color:#999;margin:0">Frames are pulled from pages using the <strong>Space Frame</strong> page template, ordered by the <strong>Space Order</strong> ACF field. Fallback frames display if no space pages exist.</p>',
    ...
    protected function render() {
        ...
        <section class="moods" aria-label="The space">
          <p class="moods__eyebrow"><?php echo esc_html( $s['eyebrow'] ); ?></p>

          <div class="moods__bleed-text" aria-hidden="true">
            <span class="moods__bleed-line"><?php echo esc_html( $s['bleed_line_1'] ); ?></span>
            <span class="moods__bleed-line"><?php echo esc_html( $s['bleed_line_2'] ); ?></span>
          </div>
                    ...
                    <div class="moods__artwork" style="background: <?php echo esc_attr( $bg_val ); ?>">
                      ...
                      <div class="moods__title-bar">
                        <p class="moods__label-mode">
                          <?php echo esc_html( $mode ); ?><span class="moods__label-sep"> · </span><?php echo esc_html( $time ); ?>
                        </p>
                        <h3 class="moods__label-title"><?php the_title(); ?></h3>
                        <span class="moods__label-cta"><?php echo esc_html( $cta_text ); ?> →</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="moods__frame-art">
                  <div class="moods__mat">
                    <div class="moods__artwork" style="background: <?php echo esc_attr( $f['bg'] ); ?>">
                      ...
                      <div class="moods__title-bar">
                        <p class="moods__label-mode">
                          <?php echo esc_html( $f['mode'] ); ?><span class="moods__label-sep"> · </span><?php echo esc_html( $f['time'] ); ?>
                        </p>
                        <h3 class="moods__label-title"><?php echo esc_html( $f['title'] ); ?></h3>
                        <span class="moods__label-cta"><?php echo esc_html( $f['cta'] ); ?> →</span>
                      </div>
                    </div>
                  </div>
                </div>
