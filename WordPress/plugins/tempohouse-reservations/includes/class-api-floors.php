...
class THR_API_Floors {
    ...
    public function upload_background( WP_REST_Request $req ): WP_REST_Response|WP_Error {
        ...
        if ( ! in_array( $file['type'], $allowed_types, true ) ) {
            return THR_API::error( 'thr_invalid_file', 'Only JPEG, PNG, WebP, or PDF files are accepted.', 422 );
        }
        ...
        $wpdb->update( $this->table,
            [ 'background_url' => $result['url'], 'updated_at' => THR_API::now_utc() ],
            [ 'id' => (int) $req->get_param( 'id' ) ]
        );
        return new WP_REST_Response( [ 'url' => $result['url'] ] );
    }
